...
public class Users
{
    ...
    async Task<IResult> CreateEmployee(HttpContext context, CreateEmployeeRequest createEmployeeRequest)
    {
        try
        {
            ...
            if (reader != null)
            {
                await using var cmd2 = Db.CreateCommand("INSERT INTO users (firstname, lastname, username, password, role, email, company) VALUES (@firstname, @lastname, @username, @password, @role::role, @email, @company_id) RETURNING id;");
                ...
                try
                {
                    var insertId = await cmd2.ExecuteScalarAsync();
                    if (insertId != null)
                    {
                        return Results.Ok(new { message = "User registered.", id = insertId });
                    }
                    else
                    {
